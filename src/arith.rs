...
mod __parse__Stmt {
    ...
    //   Term = Float (*) [";"]
    //
    //   "*" -> Reduce(Term = Float => Call(ActionFn(11));)
    //   "+" -> Reduce(Term = Float => Call(ActionFn(11));)
    //   "-" -> Reduce(Term = Float => Call(ActionFn(11));)
    //   ";" -> Reduce(Term = Float => Call(ActionFn(11));)
    //
    ...
    //   Term = Int (*) [";"]
    //
    //   "*" -> Reduce(Term = Int => Call(ActionFn(10));)
    //   "+" -> Reduce(Term = Int => Call(ActionFn(10));)
    //   "-" -> Reduce(Term = Int => Call(ActionFn(10));)
    //   ";" -> Reduce(Term = Int => Call(ActionFn(10));)
    //
    ...
    //
    //   "*" -> Shift(S23)
    //   "+" -> Reduce(Exp = MulExp => Call(ActionFn(4));)
    //   "-" -> Reduce(Exp = MulExp => Call(ActionFn(4));)
    //   ";" -> Reduce(Exp = MulExp => Call(ActionFn(4));)
    //
    //   MulOp -> S22
    ...
    //   MulExp = Term (*) [";"]
    //
    //   "*" -> Reduce(MulExp = Term => Call(ActionFn(8));)
    //   "+" -> Reduce(MulExp = Term => Call(ActionFn(8));)
    //   "-" -> Reduce(MulExp = Term => Call(ActionFn(8));)
    //   ";" -> Reduce(MulExp = Term => Call(ActionFn(8));)
    //
    ...
    //   Term = Var (*) [";"]
    //
    //   "*" -> Reduce(Term = Var => Call(ActionFn(12));)
    //   "+" -> Reduce(Term = Var => Call(ActionFn(12));)
    //   "-" -> Reduce(Term = Var => Call(ActionFn(12));)
    //   ";" -> Reduce(Term = Var => Call(ActionFn(12));)
    //
    ...
    //   Int = r#"-?[0-9]+"# (*) [";"]
    //
    //   "*" -> Reduce(Int = r#"-?[0-9]+"# => Call(ActionFn(14));)
    //   "+" -> Reduce(Int = r#"-?[0-9]+"# => Call(ActionFn(14));)
    //   "-" -> Reduce(Int = r#"-?[0-9]+"# => Call(ActionFn(14));)
    //   ";" -> Reduce(Int = r#"-?[0-9]+"# => Call(ActionFn(14));)
    //
    ...
    //   AddOp = "+" (*) [r#"[A-Za-z_][0-9A-Za-z_]*"#]
    //
    //   "(" -> Reduce(AddOp = "+" => Call(ActionFn(5));)
    //   r#"-?[0-9]+"# -> Reduce(AddOp = "+" => Call(ActionFn(5));)
    //   r#"-?[0-9]+\\.[0-9]+"# -> Reduce(AddOp = "+" => Call(ActionFn(5));)
    //   r#"[A-Za-z_][0-9A-Za-z_]*"# -> Reduce(AddOp = "+" => Call(ActionFn(5));)
    //
    pub fn __state19<
        'input,
        __TOKENS: Iterator<Item=Result<(usize, (usize, &'input str), usize),__ParseError<usize,(usize, &'input str),()>>>,
    ...
    //   Stmt = Var "=" Exp ";" (*) [EOF]
    //
    //   EOF -> Reduce(Stmt = Var, "=", Exp, ";" => Call(ActionFn(1));)
    //
    pub fn __state21<
        'input,
        __TOKENS: Iterator<Item=Result<(usize, (usize, &'input str), usize),__ParseError<usize,(usize, &'input str),()>>>,
    ...
    //   MulOp = "*" (*) [r#"[A-Za-z_][0-9A-Za-z_]*"#]
    //
    //   "(" -> Reduce(MulOp = "*" => Call(ActionFn(9));)
    //   r#"-?[0-9]+"# -> Reduce(MulOp = "*" => Call(ActionFn(9));)
    //   r#"-?[0-9]+\\.[0-9]+"# -> Reduce(MulOp = "*" => Call(ActionFn(9));)
    //   r#"[A-Za-z_][0-9A-Za-z_]*"# -> Reduce(MulOp = "*" => Call(ActionFn(9));)
    //
    pub fn __state23<
        'input,
        __TOKENS: Iterator<Item=Result<(usize, (usize, &'input str), usize),__ParseError<usize,(usize, &'input str),()>>>,
    ...
    //
    //   "*" -> Shift(S23)
    //   "+" -> Reduce(Exp = Exp, AddOp, MulExp => Call(ActionFn(3));)
    //   "-" -> Reduce(Exp = Exp, AddOp, MulExp => Call(ActionFn(3));)
    //   ";" -> Reduce(Exp = Exp, AddOp, MulExp => Call(ActionFn(3));)
    //
    //   MulOp -> S22
    ...
    //   MulExp = MulExp MulOp Term (*) [";"]
    //
    //   "*" -> Reduce(MulExp = MulExp, MulOp, Term => Call(ActionFn(7));)
    //   "+" -> Reduce(MulExp = MulExp, MulOp, Term => Call(ActionFn(7));)
    //   "-" -> Reduce(MulExp = MulExp, MulOp, Term => Call(ActionFn(7));)
    //   ";" -> Reduce(MulExp = MulExp, MulOp, Term => Call(ActionFn(7));)
    //
    ...
    //   Term = "(" Exp ")" (*) [";"]
    //
    //   "*" -> Reduce(Term = "(", Exp, ")" => Call(ActionFn(13));)
    //   "+" -> Reduce(Term = "(", Exp, ")" => Call(ActionFn(13));)
    //   "-" -> Reduce(Term = "(", Exp, ")" => Call(ActionFn(13));)
    //   ";" -> Reduce(Term = "(", Exp, ")" => Call(ActionFn(13));)
    //
    ...
    //   Stmt = "var" Var "=" Exp ";" (*) [EOF]
    //
    //   EOF -> Reduce(Stmt = "var", Var, "=", Exp, ";" => Call(ActionFn(2));)
    //
    pub fn __state41<
        'input,
        __TOKENS: Iterator<Item=Result<(usize, (usize, &'input str), usize),__ParseError<usize,(usize, &'input str),()>>>,
