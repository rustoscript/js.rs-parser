...
fn multi_binop_exprs_no_grouping() {
    ...
    assert_eq!("-10 / num - 17", format_exp!(exp!(Float(-10.0), Slash, var!("num")), Minus, Float(17.0)));
    assert_eq!("-10 * 18.5 + some_num", format_exp!(exp!(Float(-10.0), Star, Float(18.5)), Plus, var!("some_num")));
    assert_eq!("anotherNumber - _x / 17", format_exp!(var!("anotherNumber"), Minus, exp!(var!("_x"), Slash, Float(17.0))));
...
fn multi_binop_exprs_with_grouping() {
    ...
    assert_eq!("(-10 - 18.5) / 17", format_exp!(exp!(Float(-10.0), Minus, Float(18.5)), Slash, Float(17.0)));
    assert_eq!("-10 - (O_k + 17)", format_exp!(Float(-10.0), Minus, exp!(var!("O_k"), Plus, Float(17.0))));
    assert_eq!("-10 / (O_k * 17)", format_exp!(Float(-10.0), Slash, exp!(var!("O_k"), Star, Float(17.0))));
